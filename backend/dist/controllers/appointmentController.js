...
exports.updateAppointment = exports.deleteAppointment = exports.updateAppointmentStatus = exports.getAppointmentById = exports.getAppointments = exports.createAppointment = exports.updatePassedAppointments = void 0;
...
exports.updatePassedAppointments = updatePassedAppointments;
const createAppointment = async (req, res) => {
    try {
        ...
        for (const existingAppointment of existingAppointments) {
            ...
            const existingDuration = existingAppointment.duration ||
                existingAppointment.serviceId?.duration;
            if (!existingDuration) {
...
const updateAppointmentStatus = async (req, res) => {
    try {
        ...
        await (0, exports.updatePassedAppointments)();
        const appointment = await Appointment_1.default.findByIdAndUpdate(req.params.id, { status }, { new: true })
            .populate("customerId", "name email phone")
            .populate("serviceId", "name price duration")
            .populate("staffId", "name email phone specialization");
        if (!appointment) {
...
const updateAppointment = async (req, res) => {
    try {
        ...
        if (serviceId) {
            ...
            if (amount === undefined)
                resolvedAmount = service.price;
        }
        else {
            if (duration === undefined)
                resolvedDuration = appointment.duration;
            if (amount === undefined)
                resolvedAmount = appointment.amount;
        }
        ...
        if (time)
            appointment.time = time.replace(".", ":");
        if (status)
            appointment.status = status;
        if (resolvedDuration !== undefined)
            appointment.duration = resolvedDuration;
        if (resolvedAmount !== undefined)
            appointment.amount = resolvedAmount;
        await appointment.save();
        const populated = await Appointment_1.default.findById(id)
            .populate("customerId", "name email phone")
            .populate("serviceId", "name price duration")
            .populate("staffId", "name email phone specialization");
        return res.status(200).json({
